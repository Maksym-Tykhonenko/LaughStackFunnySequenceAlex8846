...
function App() {
  const [initialUrl, setInitialUrl] = useState('https://swift-zone-box.top/');
  const [initialId, setInitialId] = useState('VBN1w0zZ');
  const [oneSignKkkk, setOneSignKkkk] = useState('688d69bc-0c61-4222-9606-472398aa1f33');
  const [appsId, setAppsId] = useState('6776123837');
  const [appsFlyerDevKey, setAppsFlyerDevKey] = useState('S2XSefMbc2XcYGFTcekTfc');
...
const AppMainHlsjcidsielFlow = ({
  ...
  const APS_DEV_KEY = appsFlyerDevKey;
  const APP_ID = appsId;
  ...
  const Route = ({isFatch}) => {
    ...
    return (
      <Stack.Navigator screenOptions={{headerShown: false}}>
        <Stack.Screen name="AppMainFlow" component={AppMainFlow} />
      </Stack.Navigator>
    );
  };
  ...
  return (
    <NavigationContainer>
      {!isLoading ? <Loader /> : <Route isFatch={route} />}
    </NavigationContainer>
  );
